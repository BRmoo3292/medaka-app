...
async def predict(file: UploadFile):
    ...
    try:
        ...
        print(f"🖼️  Original resolution: {original_resolution}")
        
        # 288×516にリサイズ（アスペクト比は無視して強制リサイズ）
        resized_image = image.resize((288, 516), Image.Resampling.LANCZOS)
        
        # 圧縮された画像をバイト列に変換
        compressed_buffer = io.BytesIO()
        resized_image.save(compressed_buffer, format='JPEG', quality=85, optimize=True)
        compressed_content = compressed_buffer.getvalue()
        
        timestamps['compression'] = (time.time() - compression_start) * 1000
        
        # 圧縮後のファイルサイズ
        compressed_size_kb = len(compressed_content) / 1024
        compression_ratio = (original_size_kb - compressed_size_kb) / original_size_kb * 100
        print(f"📦 Compressed size: {compressed_size_kb:.1f}KB (-{compression_ratio:.1f}%)")
        print(f"🎯 Target resolution: 288×516")
        ...
        print("="*50)
        print(f"📤 Upload size: {compressed_size_kb:.1f}KB (was {original_size_kb:.1f}KB)")
        print(f"📥 Response size: {response_size_kb:.1f}KB")
        print(f"🌐 Total data: {(compressed_size_kb + response_size_kb):.1f}KB")
        print(f"🎯 Resolution: {original_resolution} → 288×516")
        print("-" * 30)
        ...
        if timestamps['http_request'] > 0:
            total_data_mb = (compressed_size_kb + response_size_kb) / 1024
